<main class="page-home h-full w-full">
    ...
    >
        ...
        @foreach ($heroItems as $item)
            ...
            >
                <div class="absolute inset-0 hidden bg-gradient-to-r from-gray-50 from-40% md:block"></div>
                <div
                ...
                >
                    <div class="mb-16 md:mb-0 lg:my-auto lg:pr-5">
                        ...
                        <ul class="mb-5 pr-5 text-base text-gray-700 md:text-lg">
                            <li>{{ $item->description }}</li>
                        </ul>

                        <div class="flex items-center gap-4">
                            @if (filled($item->cta_label))
                                <x-filament::button size="xl" color="primary" tag="a" :href="$item->cta_url ?? '#'">
                                    {{ $item->cta_label }}
                                </x-filament::button>
                            @endif

                            @if (filled($item->cta2_label))
                                <x-filament::link size="xl" color="primary" tag="a" :href="$item->cta2_url ?? '#'">
                                    {{ $item->cta2_label }}
                                </x-filament::link>
                            @endif
                        </div>
    ...
    <section id="features" class="container grid gap-4 pt-16 md:grid-cols-3 md:gap-8">
    ...
    <section id="clientele" class="container space-y-6 py-16">
        <h3 class="text-center text-2xl font-semibold md:text-4xl">{{ __('pages/home.content.clientele.title') }}</h3>
        <div
            x-data="{}"
            x-init="
                $nextTick(() => {
            ...
            <ul x-ref="items" class="flex animate-infinite-scroll items-center justify-center md:justify-start">
                @foreach ($clientItems as $client)
                    <li class="mx-2 md:mx-8">
                        <img class="w-16 max-w-none md:w-28" src="{{ $client->logo }}" alt="{{ $client->name }}" />
                    </li>
                @endforeach
            </ul>
        </div>
    ...
    <section id="contact" class="!mt-0 bg-white pb-16">
        <div class="container flex flex-col items-center justify-between gap-6 md:flex-row md:gap-12">
            ...
            >
                <div class="space-y-4 rounded-xl bg-white p-4 md:p-6">
                    <h4 class="text-center text-2xl font-semibold md:text-4xl">{{ config('app.name') }}</h4>
                    <div class="flex items-center gap-4">
                        <x-filament::icon icon="heroicon-o-phone" class="h-6 w-6" />
                        <div class="text-lg">
                            Call / Whatsapp
                            <a href="tel:+1-631-731-3526" class="cursor-pointer text-nowrap">+1-631-731-3526</a>
                        </div>
                    </div>
                    <div class="flex items-center gap-4">
                        <x-filament::icon icon="heroicon-o-envelope" class="h-6 w-6" />
                        <a href="mailto://support@kudvo.com" class="cursor-pointer text-lg">support@kudvo.com</a>
                    </div>
