...
class BallotSetup extends ElectionPage
{
    ...
    protected function getCreatePositionAction(): CreateAction
    {
        return CreateAction::make(name: 'createPosition')
            ...
            ->mutateFormDataUsing(callback: function (array $data, self $livewire): array {
                if (! $livewire->getElection()->voting_method->canHavePositionQuota()) {
                    $data['quota'] = 1;
                    $data['threshold'] = 1;
                    $data['abstain'] = true;
                }
                ...
                RepeatableEntry::make(name: 'positions')
                    ...
                    ->schema(components: [
                        Section::make(heading: fn (Position $state): ?string => $state->name)
                            ->compact()
                            ->description(
                                description: fn (Position $state, self $livewire): ?string => $livewire->getElection()->voting_method->canHavePositionQuota()
                                    ? collect(value: [
