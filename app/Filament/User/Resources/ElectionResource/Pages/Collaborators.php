...
class Collaborators extends ElectionPage implements HasTable
{
    ...
    public function getPermissionsSchema(): array
    {
        return [
            ...
            Radio::make(name: 'ballot_link_blasts')
                ->columns(columns: 3)
                ->label(label: __('filament.user.election-resource.pages.collaborators.form.permissions.ballot_link_blasts.label'))
                ->options(options: ElectionCollaboratorPermission::class)
                ->required(),

            Radio::make(name: 'booth_tokens')
                ->columns(columns: 3)
                ->label(label: __('filament.user.election-resource.pages.collaborators.form.permissions.booth_tokens.label'))
                ->options(options: ElectionCollaboratorPermission::class)
                ->required(),
